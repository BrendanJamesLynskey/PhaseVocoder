...
- Stochastic residual synthesis for full-bandwidth time-stretching
...
3. The inverse CWT reconstructs the wavelet-captured component, scaled by a least-squares factor. The residual (energy the CWT doesn't capture) is time-stretched using stochastic synthesis — its spectral shape and amplitude envelope are interpolated to the target length, then fresh noise is generated with the correct spectral colour and temporal contour. This avoids pitch-shifting or thinning artefacts that simpler resampling methods would introduce.
